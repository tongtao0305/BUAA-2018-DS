...
void printpath(int v0,int v1){
	...
	strcpy(string,bjvertex[board[bcount-1]].name);
	fprintf(out,"%s-%d(",string,line);
	scount=1; 
	for(i=bcount-2;i>0;i--,scount++){
		if(bjweights[board[i]][board[i-1]].id!=line){
			line=bjweights[board[i]][board[i-1]].id;
			strcpy(string,bjvertex[board[i]].name);
			fprintf(out,"%d)-%s-%d(",scount,string,line);
			scount=0;
		}
	}
	strcpy(string,bjvertex[board[i]].name);
...
int main(void) {   
	FILE *in;
	int temp,i,j,k,v0,v1;
	char start[100],end[100]; 
	in=fopen("tt.txt","r");
	creategraph();
	while(fscanf(in,"%s %s %d",end,start,&temp)!=EOF){
		for(v0=0;v0<Vnum;v0++)
